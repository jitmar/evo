...
TEST_F(OrganismTest, Constructor) {
    Organism organism(test_bytecode_, vm_);
...
TEST_F(OrganismTest, Replication) {
    Organism parent(test_bytecode_, vm_);
    parent.setFitnessScore(0.8);
    
    auto child = parent.replicate(vm_, 0.1, 2);
...
TEST_F(OrganismTest, Serialization) {
    ...
    Organism new_organism({}, vm_);
    EXPECT_TRUE(new_organism.deserialize(serialized_str, vm_));
...
TEST_F(OrganismTest, CopyConstructor) {
    Organism original(test_bytecode_, vm_);
...
TEST_F(OrganismTest, AssignmentOperator) {
    ...
    Organism assigned(other_bytecode, vm_);
...
TEST_F(OrganismTest, MultipleReplications) {
    ...
    for (int i = 0; i < 5; ++i) {
        children.push_back(organism.replicate(vm_, 0.05, 1));
    }
...
TEST_F(OrganismTest, MutationRate) {
    ...
    auto child1 = organism.replicate(vm_, 0.0, 0);
    EXPECT_EQ(child1->getBytecode(), organism.getBytecode());
    
    // Test with high mutation rate
    auto child2 = organism.replicate(vm_, 1.0, 10);
